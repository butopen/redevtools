...
export class Suggester {
  ...
  private classesFor(className: string) {
    ...
    if (t0 == 'text') {
      ...
      } else if (colors.includes(t1)) suggestedClasses = colorSizes.map(t0t1Class);
      else if (sizes.includes(t1)) suggestedClasses = sizes.map(t0t1Class);
      else if (!t2) {
      ...
      else if (borderCorners.includes(t1)) suggestedClasses = colorSizes.map(t0t1Class);
      else if (!t2) {
        suggestedClasses = starting(colors, t1).map(t0ClassWithDash);
      }
