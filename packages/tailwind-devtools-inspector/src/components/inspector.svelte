<script lang="ts">
    ...
    import {throttle} from "../shared/throttle.util";
    import TargetDescriptor from "./target-descriptor.svelte";
    import {findLastTarget} from "../shared/target.util";
    import {inspectStore, updateInspectedTarget, updatePreviewTarget} from "../stores/inspect.store";
    import {delayed} from "../shared/delayed.util";

    let lastTarget: HTMLDivElement
    ...
    function updateBox(target) {
        delayed(() => {
            ...
            if (box && inspector) {
                inspector.style.width = box.width + 'px'
                inspector.style.height = box.height + 'px'
                inspector.style.top = box.top + 'px'
                inspector.style.left = box.left + 'px'
            }
        })
    }

    inspectStore.subscribe(t => {
        if (t.previewTarget)
            updateBox(t.previewTarget)
    })

    function onMouseover(e: MouseEvent) {
        if ($inspectStore.showInspectorOnMouseMove)
            lastTarget = findLastTarget(e)
    }
    ...
    const onMousemove = throttle((e: MouseEvent) => {
        if ($inspectStore.showInspectorOnMouseMove) {
            if (!lastTarget)
                lastTarget = findLastTarget(e)
            updatePreviewTarget(lastTarget)
        }
    }, 50)

    const onClick = e => {
        if ($inspectStore.showInspectorOnMouseMove) {
            updateInspectedTarget(lastTarget)
            e.preventDefault()
        }
    }
...
{#if $inspectStore.previewTarget}

    <div class="redevtools inspector" bind:this={inspector}>
        <svg
...
<style lang="scss">
  ...
  .inspector {
    @apply flex flex-col items-center justify-center fixed pointer-events-none;
    z-index: 10001;
